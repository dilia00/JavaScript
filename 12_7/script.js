function creatItem(data) {
    data.forEach((item) => {
        productsItems.insertAdjacentHTML('beforeend', getCard(item.title, item.price, item.desc, item.image, item.id))
    })
}

function getCard(title, price, desc, image, id) {
    return `
    <div class="products-card">
        <div class="products__img">
            ...
            <div class="products-card__overlay">
                <button class="products-card__btn" id="${id}">
                    <svg
                        class="products-card__svg"
        ...
        <div class="products__desc">
            <a href="product.html" class="products__desc-link">
            ${title}
            </a>
            <p class="products__desc-text">
            ${desc}
            </p>
            <p class="products__desc-price">$${price}</p>
        </div>
    </div>`
}

function addCard(data, e) {
    data.forEach(el => {
        if (e.target.id === el.id) {
            createProductCard(el, cartEl);
        }
    });
}
...
const productsCardBtn = document.querySelectorAll('.products-card__btn');
let ListIdSelectedProducts = [];
let delEl;

productsCardBtn.forEach(element => {

    element.addEventListener('click', function (e) {
        if (!cartEl.className.includes("cart_on")) {
            cartEl.classList.add("cart_on");
            addCard(data, e);
            ListIdSelectedProducts.push(e.target.id);

        } else if (ListIdSelectedProducts.length) {
            if (ListIdSelectedProducts.includes(e.target.id)) {
                cartEl.querySelector(`#cart__quantity_${e.target.id}`).value++;
            } else {
                addCard(data, e);
                ListIdSelectedProducts.push(e.target.id);
            }
...
cartEl.addEventListener('click', function (e) {
    if (e.target.classList.contains("cart-goods__close")) {
        delEl = e.target.parentNode.classList[1];
        e.target.parentNode.remove();
    } else if (e.target.tagName == "svg") {
        delEl = e.target.parentNode.parentNode.classList[1];
        e.target.parentNode.parentNode.remove();
    } else if (e.target.tagName == "path") {
        delEl = e.target.parentNode.parentNode.parentNode.classList[1];
        e.target.parentNode.parentNode.parentNode.remove();
    }

    const filteredList = ListIdSelectedProducts.filter((number) => number !== delEl);
    ListIdSelectedProducts = filteredList;

    const cartGoodsEl = document.querySelectorAll('.cart-goods');
    if (!cartGoodsEl.length) {
        cartEl.classList.remove("cart_on");
    }
});
